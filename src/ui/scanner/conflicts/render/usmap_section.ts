...
export function renderHookItem(diag: UsmapHookDiagnostic): string {
  ...
  if (diag.category === 'palschema') {
    categoryBadge = `
      <span style="font-size: var(--text-2xs, 10px); font-weight: 700; color: var(--accent); background: rgba(0,210,255,0.1); border: 1px solid rgba(0,210,255,0.25); border-radius: 4px; padding: calc(2px * var(--ui-scale, 1)) calc(6px * var(--ui-scale, 1));">
        📊 ${escapeHtml(t('scanner.usmap_category_palschema') || 'PalSchema')}
      </span>
    `;
  } else if (diag.category === 'pak') {
    categoryBadge = `
      <span style="font-size: var(--text-2xs, 10px); font-weight: 700; color: var(--warning); background: rgba(255,170,0,0.1); border: 1px solid rgba(255,170,0,0.25); border-radius: 4px; padding: calc(2px * var(--ui-scale, 1)) calc(6px * var(--ui-scale, 1));">
        📦 ${escapeHtml(t('scanner.usmap_category_pak') || 'Pak Asset')}
      </span>
    `;
  } else if (diag.category === 'anti_pattern' || diag.status === 'blind_pcall') {
    categoryBadge = `
      <span style="font-size: var(--text-2xs, 10px); font-weight: 700; color: #f87171; background: rgba(248,113,113,0.1); border: 1px solid rgba(248,113,113,0.25); border-radius: 4px; padding: calc(2px * var(--ui-scale, 1)) calc(6px * var(--ui-scale, 1));">
        🛡️ ${escapeHtml(t('scanner.usmap_category_antipattern') || 'Anti-Pattern')}
      </span>
    `;
  } else {
    categoryBadge = `
      <span style="font-size: var(--text-2xs, 10px); font-weight: 700; color: #a78bfa; background: rgba(167,139,250,0.1); border: 1px solid rgba(167,139,250,0.25); border-radius: 4px; padding: calc(2px * var(--ui-scale, 1)) calc(6px * var(--ui-scale, 1));">
  ...
  if (diag.resolved) {
    statusBadge = `
      <span style="font-size: var(--text-2xs, 10px); font-weight: 700; color: var(--success); background: rgba(76,175,80,0.15); border: 1px solid rgba(76,175,80,0.3); border-radius: 4px; padding: calc(2px * var(--ui-scale, 1)) calc(6px * var(--ui-scale, 1));">
    ...
    statusBadge = `
      <span style="font-size: var(--text-2xs, 10px); font-weight: 700; color: var(--danger); background: rgba(255,75,75,0.15); border: 1px solid rgba(255,75,75,0.3); border-radius: 4px; padding: calc(2px * var(--ui-scale, 1)) calc(6px * var(--ui-scale, 1));">
    ...
    statusBadge = `
      <span style="font-size: var(--text-2xs, 10px); font-weight: 700; color: var(--warning); background: rgba(255,170,0,0.15); border: 1px solid rgba(255,170,0,0.3); border-radius: 4px; padding: calc(2px * var(--ui-scale, 1)) calc(6px * var(--ui-scale, 1));">
    ...
    statusBadge = `
      <span style="font-size: var(--text-2xs, 10px); font-weight: 700; color: #fbbf24; background: rgba(251,191,36,0.15); border: 1px solid rgba(251,191,36,0.3); border-radius: 4px; padding: calc(2px * var(--ui-scale, 1)) calc(6px * var(--ui-scale, 1));">
    ...
    statusBadge = `
      <span style="font-size: var(--text-2xs, 10px); font-weight: 700; color: #f87171; background: rgba(248,113,113,0.15); border: 1px solid rgba(248,113,113,0.3); border-radius: 4px; padding: calc(2px * var(--ui-scale, 1)) calc(6px * var(--ui-scale, 1));">
    ...
    statusBadge = `
      <span style="font-size: var(--text-2xs, 10px); font-weight: 700; color: var(--accent); background: rgba(0,210,255,0.12); border: 1px solid rgba(0,210,255,0.3); border-radius: 4px; padding: calc(2px * var(--ui-scale, 1)) calc(6px * var(--ui-scale, 1));">
  ...
  return `
    <div class="scanner-conflict-item" style="border-left: 3px solid ${borderCol};">
      <div class="scanner-conflict-header" style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: calc(6px * var(--ui-scale, 1));">
        <div style="display: flex; align-items: center; gap: calc(8px * var(--ui-scale, 1)); flex-wrap: wrap;">
          ${categoryBadge}
          <span class="scanner-conflict-title" style="font-size: var(--text-sm, 12.5px);">${escapeHtml(diag.hookTarget)}</span>
          ${statusBadge}
        </div>
        ${!diag.suggestion ? `
          <button class="btn btn-secondary btn-xs scan-code-jump-btn" data-mod-id="${escapeHtml(diag.modId)}" data-file-path="${escapeHtml(diag.filePath)}" data-line="${diag.lineNumber}" style="font-size: var(--text-xs, 10.5px); padding: calc(2px * var(--ui-scale, 1)) calc(8px * var(--ui-scale, 1));">
            📝 ${escapeHtml(t('common.edit') || 'Edit in Code')}
          </button>
        ` : ''}
      </div>
      <div style="display: flex; justify-content: space-between; align-items: center; font-size: var(--text-xs, 11px); margin-top: 4px; flex-wrap: wrap; gap: 4px;">
        <span style="color: var(--text-primary); font-weight: 600;">📦 ${escapeHtml(diag.modName)} <span style="font-family: monospace; font-size: var(--text-xs, 10.5px); color: var(--text-muted); font-weight: normal;">(${escapeHtml(diag.filePath)}:L${diag.lineNumber})</span></span>
        <span style="font-size: var(--text-xs, 10.5px); color: ${isBroken ? 'var(--danger)' : 'var(--text-muted)'};">${escapeHtml(diag.reason)}</span>
      </div>
      ${diag.suggestion ? `
        <div style="background: rgba(0, 210, 255, 0.08); border: 1px solid rgba(0, 210, 255, 0.25); border-radius: 6px; padding: calc(6px * var(--ui-scale, 1)) calc(12px * var(--ui-scale, 1)); margin-top: 8px; display: flex; align-items: center; justify-content: space-between; gap: calc(10px * var(--ui-scale, 1)); flex-wrap: wrap;">
          <div style="display: flex; align-items: center; gap: calc(8px * var(--ui-scale, 1)); font-size: var(--text-xs, 11px);">
          ...
          <button class="btn btn-primary btn-xs scan-code-jump-btn" data-mod-id="${escapeHtml(diag.modId)}" data-file-path="${escapeHtml(diag.filePath)}" data-line="${diag.lineNumber}" style="font-size: var(--text-xs, 10.5px); padding: calc(3px * var(--ui-scale, 1)) calc(10px * var(--ui-scale, 1)); font-weight: 700;">
...
export function buildUsmapSectionHtml(res: ScanResult): string {
  ...
  return `
    <details class="scanner-card-section" style="width: 100%; margin-bottom: calc(20px * var(--ui-scale, 1)); border-color: ${isClean ? 'rgba(76, 175, 80, 0.3)' : 'rgba(255, 75, 75, 0.3)'};" open>
      <summary class="scanner-card-header" style="outline: none; display: flex; align-items: center; justify-content: space-between; background: ${isClean ? 'rgba(76, 175, 80, 0.04)' : 'rgba(255, 75, 75, 0.04)'};">
        <span style="color: ${isClean ? 'var(--success)' : 'var(--danger)'}; font-weight: 700; font-size: var(--text-base, 13.5px); display: flex; align-items: center; gap: calc(8px * var(--ui-scale, 1));">
          <span>⚡</span>
      ...
      <div class="scanner-card-body" style="gap: calc(12px * var(--ui-scale, 1)); padding: calc(14px * var(--ui-scale, 1));">
        <div style="display: flex; gap: calc(12px * var(--ui-scale, 1)); align-items: center; flex-wrap: wrap;">
          <div style="padding: calc(6px * var(--ui-scale, 1)) calc(12px * var(--ui-scale, 1)); background: rgba(76, 175, 80, 0.1); border: 1px solid rgba(76, 175, 80, 0.25); border-radius: 4px; font-size: var(--text-xs, 11px); font-weight: 700; color: var(--success);">
            ✅ ${d.validHooks} ${escapeHtml(t('scanner.usmap_valid_hooks') || 'Valid Hooks')}
          </div>
          ${d.brokenHooks > 0 ? `
            <div style="padding: calc(6px * var(--ui-scale, 1)) calc(12px * var(--ui-scale, 1)); background: rgba(255, 75, 75, 0.12); border: 1px solid rgba(255, 75, 75, 0.3); border-radius: 4px; font-size: var(--text-xs, 11px); font-weight: 700; color: var(--danger);">
              ⚠️ ${d.brokenHooks} ${escapeHtml(t('scanner.usmap_broken_hooks') || 'Broken / Obsolete Hooks Detected')}
            </div>
          ` : ''}
          <div style="font-size: var(--text-xs, 11px); color: var(--text-muted); margin-left: auto;">
