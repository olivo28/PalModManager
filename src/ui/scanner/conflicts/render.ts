...
export async function renderConflictsPanel(container: HTMLElement): Promise<void> {
  ...
  // USMAP Unreal Engine Schema Hook Diagnostics Section
  let usmapSectionHtml = '';
  if (res.usmapDiagnostics && res.usmapDiagnostics.hasUsmap) {
    const d = res.usmapDiagnostics;
    const isClean = d.brokenHooks === 0;

    const isItemBroken = (diag: UsmapHookDiagnostic) =>
      diag.status === 'broken_class' ||
    ...
    const renderHookItem = (diag: UsmapHookDiagnostic) => {
