...
#[tauri::command]
pub fn set_ui_scale(scale: f64, state: State<AppState>) -> Result<Value, String> {
    let mut data = state.data.lock().map_err(|e| e.to_string())?;
    data.settings.ui_scale = Some(scale);
    let result = serde_json::to_value(&data.settings).map_err(|e| e.to_string())?;
    let data_clone = data.clone();
    drop(data);
    let _ = db::save_db(&data_clone.settings.program_path, &data_clone);
...
#[tauri::command]
pub fn set_language(language: String, state: State<AppState>) -> Result<Value, String> {
    let mut data = state.data.lock().map_err(|e| e.to_string())?;
    data.settings.language = Some(language);
    let result = serde_json::to_value(&data.settings).map_err(|e| e.to_string())?;
    let data_clone = data.clone();
    drop(data);
    let _ = db::save_db(&data_clone.settings.program_path, &data_clone);
...
#[tauri::command]
pub fn set_folder_expand_mode(mode: String, state: State<AppState>) -> Result<Value, String> {
    let mut data = state.data.lock().map_err(|e| e.to_string())?;
    data.settings.folder_expand_mode = Some(mode);
    let result = serde_json::to_value(&data.settings).map_err(|e| e.to_string())?;
    let data_clone = data.clone();
    drop(data);
    let _ = db::save_db(&data_clone.settings.program_path, &data_clone);
